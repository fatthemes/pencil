	...
	$pencil_slider_posts_array = get_posts( $pencil_slider_posts_array_args ); ?>
		<div class="container-fluid">
			<div class="row">
				...
				<?php foreach ( $pencil_slider_posts_array as $pencil_slide ) : ?>
						<div class="pencil-featured-slider-wrapper">
							<?php if ( has_post_thumbnail( $pencil_slide->ID ) ) : ?>
							
								<?php $pencil_wp_get_attachment_image_src = wp_get_attachment_image_src( get_post_thumbnail_id( $pencil_slide->ID ), get_theme_mod( 'home_page_slider_img_size', 'full' ) ); ?>
							...
							<h2 class="pencil-featured-slider-header"><a href="<?php the_permalink( $pencil_slide->ID ); ?>" rel="bookmark"><?php echo wp_kses_post( get_the_title( $pencil_slide->ID ) ); ?></a></h2>
							</div>
							</div>
							<?php else : ?>
